        ...
        if (FirebaseAuth.getInstance().getCurrentUser() != null) {
            ...
            if (favoritos != null && !favoritos.isEmpty()) {
                for (String favorito : favoritos) {
                    ...
                    if (datosLibro.length == 6) { // Asegúrate de que tenga 6 elementos para incluir el rating
                        try {
                            String title = datosLibro[0];
                            String imageUrl = datosLibro[1];
                            String description = datosLibro[2];
                            String category = datosLibro[3];
                            float averageRating = Float.parseFloat(datosLibro[4]); // Obtener el rating
                            int id = Integer.parseInt(datosLibro[5]);

                            List<Review> reviews = new ArrayList<>();

                            Book book = new Book(title, "", description, imageUrl, category, averageRating, reviews, id);
